...
public class ConfigurationMenuContributor : IMenuContributor
{
    ...
    private static void AddMenuItemHQTasks(MenuConfigurationContext context, ApplicationMenuItem parentMenu)
    {
        parentMenu.AddItem(
            new ApplicationMenuItem(
                Menus.ConfigurationMenus.HQTasks,
                context.GetLocalizer<ConfigurationResource>()["Menu:HQTasks"],
                "/Configuration/HQTasks",
                icon: "fa fa-file-alt",
                requiredPermissionName: ConfigurationPermissions.HQTasks.Default
            )
        );
    }
}