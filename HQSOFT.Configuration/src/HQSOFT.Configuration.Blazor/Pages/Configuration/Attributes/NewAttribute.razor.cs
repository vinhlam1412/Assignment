    ...
    public partial class NewAttribute
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (EditingAttributeId != Guid.Empty)
            {
                ...
                SetStatChange(EditingAttribute.ControlType);
            }
        ...
        void Grid_EditModelSaving(GridEditModelSavingEventArgs e)
        {
            ...
            if (e.IsNew)
            {
                ...
                CheckListAttriDetailAsync((CSAttributeDetailDto)e.DataItem, edittableDetail);
            }
            else
            {
        ...
        public Task CheckListAttriDetailAsync(CSAttributeDetailDto dataItem, CSAttributeDetailDto newDataItem)
        {
            // Find ValueID in list
            var index = listAttribute.FindIndex(r => r.ValueID == newDataItem.ValueID);
            if (index != -1)
            {
                //Delete list[index] in database
                AttributeDetailsAppService.DeleteAsync(listAttribute[index].Id);
                
                //Delete list[index] in list
                listAttribute.RemoveAt(index);
                
                //Add item existed in to list
                listAttribute.Add(newDataItem);
            }
            else
            {
                listAttribute.Add(newDataItem);
        ...
        public void SetStatChange(ControlType value)
        {
            if (value == ControlType.MultiSelectCombo || value == ControlType.Combo)
            {
